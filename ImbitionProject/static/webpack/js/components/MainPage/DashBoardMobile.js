...
class DashBoardMobileDumb extends React.Component {
  constructor(props) {
    super(props);
      ...
      if (this.props.user && this.props.user.phone) {
        props.fetchRecordSummary({
          ...
          for (let i = 5; i < data.recordsummary.order.length; i += 2) {
            ...
            counter += 1;
            if (counter % this.props.itemPerRow === 0) {
...
DashBoardMobileDumb.propTypes = {
  ...
  itemPerRow: PropTypes.func.isRequired,
};


DashBoardMobileDumb.defaultProps = {
  itemPerRow: 1,
};
...
const mapStateToProps = state => ({
  ...
  DashBoardMobile = connect(mapStateToProps, mapDispatchToProps)(DashBoardMobileDumb);
